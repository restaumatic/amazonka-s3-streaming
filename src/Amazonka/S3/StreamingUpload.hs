...
import Amazonka (HashedBody (..), getFileSize, hashedFileRange, send, toBody)
 ...
 where
  ...
  finishMultiUploadConduit upId = do
    ...
    res <-
      lift $
        send env $
          (newCompleteMultipartUpload buck k upId)
            { multipartUpload =
